...
export default function HypothesisDetail() {
  ...
  const { data: needs } = useQuery({ queryKey: ["needs-select"], queryFn: async () => { const { data } = await supabase.from("partner_needs").select("need_id, title").order("title"); return data || []; } });
  ...
  const [form, setForm] = useState({
    title: "", partner_id: "", need_id: "", unit_id: "", hypothesis_status: "new",
    confidence_level: "", relevance_score: "", rationale: "",
    recommended_entry_point: "", recommended_collaboration_format: "", notes: "",
  });
  ...
  const del = useMutation({
    ...
    onSuccess: () => { toast.success("Удалено"); navigate("/hypotheses"); },
  });
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Select value={form.need_id} onValueChange={v => set("need_id", v)} disabled={!canEdit}>
              <SelectTrigger><SelectValue placeholder="Выберите" /></SelectTrigger>
              <SelectContent>{needs?.map(n => <SelectItem key={n.need_id} value={n.need_id}>{n.title}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            <Label>Коллектив МИЭМ</Label>
          ...
          <div className="space-y-2">
            ...
            <Select value={form.hypothesis_status} onValueChange={v => set("hypothesis_status", v)} disabled={!canEdit}>
              ...
              <SelectContent>
                <SelectItem value="new">Новая</SelectItem><SelectItem value="testing">Тестируется</SelectItem>
                <SelectItem value="confirmed">Подтверждена</SelectItem><SelectItem value="rejected">Отклонена</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-2"><Label>Оценка релевантности</Label><Input type="number" value={form.relevance_score} onChange={e => set("relevance_score", e.target.value)} disabled={!canEdit} /></div>
          <div className="space-y-2 sm:col-span-2"><Label>Обоснование</Label><Textarea value={form.rationale} onChange={e => set("rationale", e.target.value)} disabled={!canEdit} rows={3} /></div>
          <div className="space-y-2"><Label>Точка входа</Label><Input value={form.recommended_entry_point} onChange={e => set("recommended_entry_point", e.target.value)} disabled={!canEdit} /></div>
          <div className="space-y-2"><Label>Формат сотрудничества</Label><Input value={form.recommended_collaboration_format} onChange={e => set("recommended_collaboration_format", e.target.value)} disabled={!canEdit} /></div>
