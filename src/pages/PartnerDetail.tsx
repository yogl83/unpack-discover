...
export default function PartnerDetail() {
  ...
  const del = useMutation({
    ...
    onSuccess: () => { toast.success("Организация удалена"); qc.invalidateQueries({ queryKey: ["partners"] }); navigate("/partners"); },; qc.invalidateQueries({ queryKey: ["partners"] }); navigate("/partners"); },; qc.invalidateQueries({ queryKey: ["partners"] }); navigate("/partners"); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold"><h1 className="text-2xl font-bold">{isNew ? "Новая организация" : form.partner_name}</h1></h1>
        {!isNew && isAdmin && (
          <ConfirmDialog title="Удалить организацию?" description="Организация и все связанные данные (контакты, потребности, гипотезы) будут удалены безвозвратно." onConfirm={() => del.mutate()} /> description="Организация и все связанные данные (контакты, потребности, гипотезы) будут удалены безвозвратно." onConfirm={() => del.mutate()} />" description="Партнёр и все связанные данные (контакты, потребности, гипотезы) будут удалены безвозвратно." onConfirm={() => del.mutate()} />
        )}
      </div>
