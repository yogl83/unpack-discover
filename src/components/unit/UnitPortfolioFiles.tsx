...
export function UnitPortfolioFiles({ unitId, editable }: Props) {
  ...
  const { data: files = [] } = useQuery({
    queryKey: ["unit-portfolio-files", unitId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("unit_portfolio_files" as any)
        .select("*")
        .eq("unit_id", unitId)
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const ts = Date.now();
      const storagePath = `${unitId}/${ts}_${file.name}`;
      const { error: uploadErr } = await supabase.storage
        .from("unit-portfolio-files")
        .upload(storagePath, file);
      if (uploadErr) throw uploadErr;

      const { error: metaErr } = await supabase.from("unit_portfolio_files" as any).insert({
        unit_id: unitId,
      ...
      qc.invalidateQueries({ queryKey: ["unit-portfolio-files", unitId] });
    } catch (err: any) {
  ...
  const handleDelete = async (f: any) => {
    ...
    await supabase.from("unit_portfolio_files" as any).delete().eq("file_id", f.file_id);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium">Файлы и документы</h3>
        {editable && (
          <>
